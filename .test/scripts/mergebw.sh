...
cmd="wiggletools mean ${script_args[@]} | grep -v "_" | grep -v "chrEBV" | awk 'NF==4{print}' > ${out_name}.tmp.bdg"
echo $cmd
wiggletools mean ${script_args[@]} | grep -v "_" | grep -v "chrEBV" | awk 'NF==4{print}' > ${out_name}.tmp.bdg
