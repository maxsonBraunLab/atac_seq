...
rule frip_count:
    ...
    shell:
        """
        rip=$(bedtools intersect -u -a {input.bamfile} -b {input.peaks} -ubam | samtools view -c); \
...
rule sample_counts:
    input:
        ...
        bam = "samples/bamfiles/{sample}_rmChrM_dedup_quality_shifted.bam"
    output:
...
rule consensus_peaks:
    ...
    shell:
        ...
        Rscript --vanilla {params.script_file} {params.merged_bed} {params.blacklist} {params.present_in_number} {output.reads_catalog_bed} {params.genome} {params.metadata}
        """
